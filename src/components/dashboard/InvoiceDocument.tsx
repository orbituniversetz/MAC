...
export function InvoiceDocument({ invoice, settings, className }: InvoiceDocumentProps) {
  ...
  const header = (
    <div className="w-full shrink-0">
      ...
      <div className="flex justify-between items-center mb-8 bg-zinc-950 text-white p-4 rounded shadow-sm shrink-0 avoid-break text-left">
        <div>
          <h2 className="text-lg font-bold uppercase tracking-widest">TAX INVOICE / RECEIPT</h2>
          <p className="text-[10px] text-zinc-400 font-bold uppercase mt-0.5">
            Date Invoiced: {invoice.createdAt ? new Date(invoice.createdAt).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }) : 'N/A'}
          </p>
        </div>
  ...
  const details = (
    <div className="grid grid-cols-2 gap-12 mb-8 shrink-0 avoid-break text-left w-full">
      <div>
        ...
        <div className="space-y-1">
          ...
          <div className="flex flex-col gap-1 mt-2">
              <p className="text-xs font-black text-zinc-800">TEL: {invoice.customerPhone}</p>
              <p className="text-xs font-black bg-zinc-100 px-2 py-0.5 rounded-full w-fit">TIN: {invoice.customerTin || 'N/A'}</p>
          </div>
        </div>
      </div>
      <div className="bg-zinc-50 p-5 rounded-2xl border border-zinc-100 flex flex-col justify-center">
        <h3 className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-3">Invoice & Vehicle Info</h3>
        <div className="space-y-2 text-sm">
          <p><span className="font-black text-zinc-400 uppercase text-[9px] tracking-widest w-28 inline-block">Invoice Date:</span> <span className="font-black text-zinc-900">{invoice.createdAt ? new Date(invoice.createdAt).toLocaleDateString() : 'N/A'}</span></p>
          <p><span className="font-black text-zinc-400 uppercase text-[9px] tracking-widest w-28 inline-block">Registration:</span> <span className="font-black text-zinc-900">{invoice.vehiclePlate || 'N/A'}</span></p>
          <p><span className="font-black text-zinc-400 uppercase text-[9px] tracking-widest w-28 inline-block">Make/Model:</span> <span className="font-bold text-zinc-700">{invoice.vehicleModel || 'N/A'}</span></p>
        </div>
      </div>
    </div>
  );
  ...
  const renderItem = (item: any) => (
    <tr className="text-xs border-b border-zinc-100">
      <td className="p-4 font-black text-zinc-900 uppercase tracking-tight text-left break-words">{item.description}</td>
  ...
  const footer = (
    <div className="w-full text-left avoid-break mt-4">
        ...
        <div className="flex justify-end">
          <div className="w-72 space-y-2 bg-zinc-50 p-6 rounded-3xl border border-zinc-100 shadow-sm">
            ...
            <div className="flex justify-between text-[10px] pt-2 text-green-700 font-black border-t border-dashed border-zinc-300">
              ...
              <span>TZS {totalPaid.toLocaleString()}</span>
            </div>
        ...
        <div className="p-6 rounded-2xl flex justify-between items-center border-2 bg-zinc-950 text-white shadow-md">
          <div>
            ...
            <h4 className="text-xl font-black tracking-tight">
              {isFullyPaid ? 'FULLY SETTLED' : isPartiallyPaid ? 'PARTIALLY PAID' : 'PENDING PAYMENT'}
            </h4>
          </div>
          <div className="text-right">
