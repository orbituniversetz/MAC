...
export function ProformaDocument({ proforma, settings, className }: ProformaDocumentProps) {
  ...
  return (
    <div id="proforma-document" className={cn("a4-page font-sans", className)}>
      <div className="flex items-center justify-between mb-4 border-b-2 border-zinc-100 pb-4 shrink-0 avoid-break">
        ...
        <div className="text-right flex flex-col justify-center">
          ...
          <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-tight flex flex-col items-end">
            ...
            <span className="text-zinc-800 font-bold">Tel: {settings.garage_phone} | TIN: {settings.garage_tin}</span>
          </div>
        </div>
      </div>

      <div className="flex justify-between items-center mb-8 bg-primary text-white p-4 rounded shadow-sm shrink-0 avoid-break">
        ...
        <div className="text-right">
          ...
          <p className="text-xl font-bold leading-none">{proforma.proformaNo}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-12 mb-8 shrink-0 avoid-break">
        <div>
          ...
          <div className="space-y-1">
            <p className="font-bold text-lg text-zinc-900 leading-tight">{proforma.customerName}</p>
            <p className="text-sm font-medium text-zinc-600">{proforma.customerAddress || 'No Address Provided'}</p>
            <p className="text-xs font-bold text-zinc-800">TIN: {proforma.customerTin || 'N/A'}</p>
            <p className="text-sm font-medium text-zinc-600">Tel: {proforma.customerPhone}</p>
          </div>
        </div>
        <div className="bg-zinc-50 p-5 rounded-2xl border border-zinc-100 flex flex-col justify-center">
          <h3 className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-2">Vehicle Details</h3>
          <div className="space-y-1">
            <p><span className="font-bold text-zinc-400 uppercase text-[9px] tracking-widest">Plate Number:</span> <span className="font-bold text-zinc-900">{proforma.vehiclePlate || 'N/A'}</span></p>
            <p><span className="font-bold text-zinc-400 uppercase text-[9px] tracking-widest">Make / Model:</span> <span className="font-medium text-zinc-700">{proforma.vehicleModel || 'N/A'}</span></p>
            {proforma.jobNo && <p className="mt-2 text-[10px] font-bold bg-white border px-2 py-0.5 rounded-full inline-block">Ref Job: {proforma.jobNo}</p>}
          </div>
        </div>
      </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-zinc-950 text-white">
              <th className="p-3 text-left text-[10px] font-bold uppercase">Description</th>
              <th className="p-3 text-center text-[10px] font-bold uppercase w-16">Qty</th>
              <th className="p-3 text-right text-[10px] font-bold uppercase w-32">Unit Price</th>
          ...
          <tbody className="divide-y divide-zinc-100">
            {proforma.items?.map((item: any) => (
              <tr key={item.id} className="text-xs avoid-break">
                ...
                <td className="p-3.5 text-right font-bold whitespace-nowrap text-zinc-900">TZS {item.subtotal.toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className={cn(
        "p-6 rounded-2xl mb-10 flex justify-between items-center border-2 shrink-0 avoid-break",
        ...
        <div className="grid grid-cols-2 gap-12">
          <div>
            ...
            <div className="space-y-1.5 text-xs">
              <p><span className="font-bold text-zinc-400 uppercase text-[9px] w-28 inline-block tracking-widest">Bank:</span> <span className="font-bold text-zinc-900">{settings.bank_name}</span></p>
              <p><span className="font-bold text-zinc-400 uppercase text-[9px] w-28 inline-block tracking-widest">Account Name:</span> <span className="font-medium text-zinc-700">{settings.bank_account_name}</span></p>
              <p><span className="font-bold text-zinc-400 uppercase text-[9px] w-28 inline-block tracking-widest">Account No:</span> <span className="font-bold text-zinc-900">{settings.bank_account_number}</span></p>
            </div>
          </div>
          <div className="text-[10px] text-zinc-500 italic leading-relaxed font-medium">
            <h3 className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-2 not-italic">Quotation Terms</h3>
            <p>This quotation is valid for 30 days. Any additional work discovered during service will be quoted separately. Management approval is required before proceeding.</p>
          </div>
        </div>
      </div>
    </div>
