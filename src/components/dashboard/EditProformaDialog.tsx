...
import { updateProformaStatus } from '@/lib/actions';

export function EditProformaDialog({ proforma }: { proforma: any }) {
  const [isOpen, setIsOpen] = useState(false);
  const [status, setStatus] = useState(proforma.status);

  async function handleUpdate() {
    await updateProformaStatus(proforma.id, status);
    setIsOpen(false);
  }
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm">
          <Edit2 className="h-4 w-4 mr-2" /> Edit Lifecycle
        </Button>
      </DialogTrigger>
